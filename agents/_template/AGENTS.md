...
<一句话说明这个 profile 的职责边界和交付标准。>

## 权威技能

阅读顺序：`AGENTS.md`（本文件）→ `SOUL.md` → `HEARTBEAT.md`（如有）→ `TOOLS.md` → 技能正文。
...
6. **执行** — <填入这个 profile 特有的执行规范/技能引用；与技能冲突时以技能更新为准。>
7. **记录** — 在退出前评论进展；若要等待人类/董事会，设置 `blocked` 并交接 `assigneeUserId`。

---

使用方法：复制本文件到 `agents/<agent-key>/AGENTS.md`，替换尖括号占位内容，删除本段说明。需要 `SOUL.md`/`TOOLS.md` 的话参照已有 profile（如 `agents/python-engineer/`）补齐。
